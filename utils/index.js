...
// Update the handler to return a proper Response for serverless functions (Edge function)
export default async function handler(req) {
  try {
    const { inputLanguage, outputLanguage, inputCode } = await req.json();

    // Check if inputCode is null or empty, throw an error if invalid
    if (!inputCode) {
      return new Response('Input code is missing', { status: 400 });
    }
